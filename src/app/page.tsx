...
export default function HomePage() {
  // leitura inicial do parâmetro q da URL (/?q=tequila)
  ...
  // carrega receitas + normaliza ingredientes_norm_set
  useEffect(() => {
    (async () => {
      ...
      for (const r of recs) {
        ...
        const terms = Array.from(
          new Set(ingRows.flatMap((i: any) => productTerms(i.produto_raw)))
        );
  ...
  const results = useMemo(() => {
    ...
    if (parts.length > 1) {
      // vários ingredientes separados por vírgula => TODOS precisam estar presentes (AND)
      return items.filter(
        (r: any) =>
  ...
  // ao clicar em uma receita, salvar o filtro atual na sessão
  ...
  return (
    <div className="space-y-6">
      {/* Barra de busca + botão Nova (botão Nova só aparece no desktop; no mobile já tem bottom nav) */}
