...
export default function EditRecipePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!active) return;
        if (!r) { alert('Receita não encontrada'); router.push('/'); return; }
  ...
  async function salvar() {
    ...
    await db.transaction('rw', db.recipes, db.recipe_ingredients, async () => {
      ...
      // Estratégia simples: apagar ingredientes antigos e recriar
    ...
    <div className="grid lg:grid-cols-[1fr_320px] gap-6">
      ...
      <div className="space-y-4">
        ...
        <div className="bg-white border rounded-xl p-4 space-y-3">
          <div className="font-semibold text-lg">Editar Receita</div>
        ...
        <div className="flex gap-2">
          ...
          <a href={`/recipe/${id}`} className="px-4 py-2 border rounded bg-white hover:bg-slate-50">
            Cancelar
          </a>
        </div>
      </div>
